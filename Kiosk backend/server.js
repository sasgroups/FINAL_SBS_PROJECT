...
async function sendKioskStatus() {
  ...
  try {
    // Fetch current raw statuses (but they will internally update stable counters)
    ...
    // Use the stable statuses returned by the endpoints
    const kioskData = {
      ...
      scaleStatus: scaleStatusRes.data.status,      // already stable
      scannerStatus: scannerStatusRes.data.status,  // already stable
      realsenseStatus: realsenseStatusRes.data.status, // already stable
    ...
    // console.log("📤 Sent stable kiosk status");
  } catch (err) {
